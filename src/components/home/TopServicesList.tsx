...
export const TopServicesList = ({categoryId}: TopServicesListProps) => {
  ...
  const {data: services, isLoading} = useQuery({
    queryKey: ['top-services', i18n.language, user?.id, categoryId],
    queryFn: async () => {
      const {data, error} = await supabase.rpc('search_services', {
        search_query: undefined,
        target_lang: i18n.language,
        category_filter: categoryId ? [categoryId] : undefined,
  ...
  const toggleBookmarkMutation = useMutation({
    ...
    onMutate: async ({serviceId, isBookmarked}) => {
      ...
      await queryClient.cancelQueries({queryKey: ['top-services', i18n.language, user?.id, categoryId]});

      // Snapshot previous value
      const previousServices = queryClient.getQueryData(['top-services', i18n.language, user?.id, categoryId]);

      // Optimistically update
      queryClient.setQueryData(['top-services', i18n.language, user?.id, categoryId], (old: any) => {
        if (!old) return old;
    ...
    onError: (err, variables, context) => {
      ...
      if (context?.previousServices) {
        queryClient.setQueryData(['top-services', i18n.language, user?.id, categoryId], context.previousServices);
      }
      Toast.show({type: 'error', text1: 'Failed to update bookmark'});
    },
    onSettled: () => {
      // Refetch to ensure consistency
      queryClient.invalidateQueries({queryKey: ['top-services', i18n.language, user?.id, categoryId]});
    },
  });
